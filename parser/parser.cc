...
bool Parser::parseLine(const string &str, vector< std::shared_ptr<VisionObject> >& visionObjs) {
    ...
    if(valid) {
        std::shared_ptr<VisionObject> obj( new VisionObject(r, theta, phi, r2, theta2, phi2, lineIndex ) );
        visionObjs.push_back( obj );
    }
    return valid;
}

bool Parser::parseGoalPost(const string &str, vector< std::shared_ptr<VisionObject> >& visionObjs) {
    ...
    if(valid) {
        std::shared_ptr<VisionObject> obj( new VisionObject(r, theta, phi, goalPostIndex ) );
        visionObjs.push_back( obj );
    }
    return valid;
}

bool Parser::parseFlag(const string &str, vector< std::shared_ptr<VisionObject> >& visionObjs) {
    bool valid = false;
    ...
    if(valid) {
        std::shared_ptr<VisionObject> obj( new VisionObject(r, theta, phi, flagIndex) );
        visionObjs.push_back( obj );
    }
    return valid;
}

bool Parser::parseBall(const string &str, vector< std::shared_ptr<VisionObject> >& visionObjs) {
    bool valid = false;
    string name;
    double r, theta, phi;
    ...
    if(valid) {
        std::shared_ptr<VisionObject> obj( new VisionObject(r, theta, phi, WO_BALL) );
        visionObjs.push_back( obj );
    }
...
bool Parser::parsePlayer(const string &str, vector< std::shared_ptr<VisionObject> >& visionObjs) {
    ...
    for(size_t i = 0; i < tokens.size(); ++i) {
        if(!(tokens[i].compare("P"))) {
            ...
            if (playerTeamName.compare(teamName) || worldModel->getUNum() != uNum) {
                ...
                if (objectIndex != -1) {
                    std::shared_ptr<VisionObject> obj( new VisionObject(r, theta, phi, objectIndex ) );
                    visionObjs.push_back( obj );
                }
            }
    ...
    if(valid && notMe) {
        ...
        if(!playerTeamName.compare(teamName)) {
            ...
            std::shared_ptr<VisionObject> obj( new VisionObject(r, theta, phi, playerIndex ) );
            visionObjs.push_back( obj );
        }
        else {
...
void Parser::processVision() {
    ...
    // Print vision info
    vector< std::shared_ptr<VisionObject> >::iterator begin( visionObjs.begin() );
    vector< std::shared_ptr<VisionObject> >::iterator end( visionObjs.end() );
