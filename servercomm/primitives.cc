...
// ashko s
void NaoBehavior::gazeboAction()
{           
    std_msgs::Float64 msg[24];

    if (bodyModel->useOmniWalk())
    {
        msg[0].data = Deg2Rad(bodyModel->getTargetAngle(EFF_H1));
        msg[1].data = -Deg2Rad(bodyModel->getTargetAngle(EFF_H2));
        for(size_t i = 2; i < 24; i++)  
            // if(i == 2 || i == 6 || i == 12 || i == 13 || i == 14 || i == 19 || i == 20 || i == 21)
            //     msg[i].data = -(bodyModel->computeTorque(i,sim_effectors_, raw_joint_angles_, raw_joint_commands_) * 0.02 + Deg2Rad(bodyModel->getCurrentAngle(i)));
            // else
            //     msg[i].data = (bodyModel->computeTorque(i,sim_effectors_, raw_joint_angles_, raw_joint_commands_) * 0.02 + Deg2Rad(bodyModel->getCurrentAngle(i)));
                msg[i].data = 0;
    }
        ...
        for(size_t i = 0; i < 24; i++)        
            if(i == 2 || i == 6 || i == 12 || i == 13 || i == 14 || i == 19 || i == 20 || i == 21)
                msg[i].data = -Deg2Rad(bodyModel->getTargetAngle(i));
                // msg[i].data = -(bodyModel->computeTorque(i) * 0.02 + Deg2Rad(bodyModel->getCurrentAngle(i)));
            else 
                msg[i].data = Deg2Rad(bodyModel->getTargetAngle(i)); 
            // msg[i].data = bodyModel->computeTorque(i) * 0.02 + Deg2Rad(bodyModel->getCurrentAngle(i));        
    ...
    for(size_t i = 0; i < 24; i++)
    {  
        // ROS_INFO("command is: %f", msg[i].data);  
        joint_command_publisher[i].publish(msg[i]);
    }    
}
// ashko e
...
string NaoBehavior::composeAction()
{
    ...
    if (bodyModel->useOmniWalk())
    {
        ...
        ss << "(lae1 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(LShoulderPitch),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(lae2 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(LShoulderRoll),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(lae3 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(LElbowYaw),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(lae4 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(LElbowRoll),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(rae1 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(RShoulderPitch),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(rae2 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(RShoulderRoll),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(rae3 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(RElbowYaw),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(rae4 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(RElbowRoll),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(lle1 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(LHipYawPitch),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(lle2 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(LHipRoll),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(lle3 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(LHipPitch),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";       

        ss << "(lle4 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(LKneePitch),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(lle5 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(LAnklePitch),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(lle6 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(LAnkleRoll),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";
        if (bodyModel->hasToe())
        {
            ss << "(lle7 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(LToePitch),
                                                       sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";
        }

        ss << "(rle1 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(RHipYawPitch),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(rle2 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(RHipRoll),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(rle3 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(RHipPitch),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(rle4 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(RKneePitch),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(rle5 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(RAnklePitch),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        ss << "(rle6 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(RAnkleRoll),
                                                   sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";

        if (bodyModel->hasToe())
        {
            ss << "(rle7 " << bodyModel->computeTorque(UTWalkJointToSimEffectors(RToePitch),
                                                       sim_effectors_, raw_joint_angles_, raw_joint_commands_) << ")";
        }
