...
SenderToServer::SenderToServer(int _agentBodyType) : nh()
{
    client_reset_world = nh.serviceClient<std_srvs::Empty>("gazebo/reset_world");
    client_reset_simulation = nh.serviceClient<std_srvs::Empty>("gazebo/reset_simulation");
    client_pause_physics = nh.serviceClient<std_srvs::Empty>("gazebo/pause_physics");
    client_unpause_physics = nh.serviceClient<std_srvs::Empty>("gazebo/unpause_physics");
    std_srvs::Empty srv;    
    client_unpause_physics.call(srv);
...
void SenderToServer::LaunchMsgLoop()
{
    ...
    msg = "";
    long i = 0;    
    double currentTime, setTime, elapsedTime;
    setTime = ros::Time::now().toSec();
    while (true)
    {        
        if(bResetMonitor)
        {  
            bResetMonitor = false;    // for removre server (ashkoofaraz)
            client_reset_simulation.call(srv); // for removre server (ashkoofaraz)
            client_reset_world.call(srv); // for removre server (ashkoofaraz)
            client_pause_physics.call(srv); // for removre server (ashkoofaraz)
            client_unpause_physics.call(srv);  // for removre server (ashkoofaraz) 
            setTime = ros::Time::now().toSec();  // for removre server (ashkoofaraz) 
        }                
        currentTime = ros::Time::now().toSec();                
        elapsedTime = currentTime - setTime;
        if(elapsedTime < 0.01)
            continue;
        behavior->getWorldModel()->setTime(ros::Time::now().toSec());// for removre server (ashkoofaraz)
        //GetMessage(msg);  // for removre server (ashkoofaraz)
        lockMutex();
        ...
        {            
            if(bResetMonitor)
            {
                ...
                {                    
                    client_reset_simulation.call(srv);
                    client_reset_world.call(srv);                    
                    client_pause_physics.call(srv);
                    // if (client_reset_world.call(srv))                
                    //     ROS_INFO("Reset world gazebo");
                    // else
                    //     ROS_ERROR("Failed to call service gazebo");
