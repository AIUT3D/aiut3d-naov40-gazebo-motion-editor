...
IncTar::getReflection(BodyModel *bodyModel) {
    ...
    return std::shared_ptr<Macro>( new IncTar(newEffIDs, newIncs) );
}
...
SetTar::getReflection(BodyModel *bodyModel) {
    ...
    return std::shared_ptr<Macro>( new SetTar(newEffIDs, newAngs) );
}
 ...
 */
std::shared_ptr<Macro> SetFoot::getReflection(BodyModel *bodyModel) {
    ...
    return std::shared_ptr<Macro> (new SetFoot(newLegIDX, newPos));
}
...
std::shared_ptr<Macro> Curve::getReflection(BodyModel *bodyModel) {
    int newLegIDX = -1;
    ...
    return std::shared_ptr<Macro> (new Curve(newLegIDX, newControlPoints));
}
...
Reset::getReflection(BodyModel *bodyModel) {
    ...
    return std::shared_ptr<Macro>( new Reset( newComponents ) );
}
...
SetScale::getReflection(BodyModel *bodyModel) {
    ...
    return std::shared_ptr<Macro>( new SetScale(newEffIDs, targetScales) );
}
 ...
 * Reflets the Macro from between left and right foot.
 */
std::shared_ptr<Macro> Stabilize::getReflection(BodyModel *bodyModel) {
...
void KeyFrame::appendMacro(std::shared_ptr<Macro> macro) {
    macros.push_back( macro );
}

std::shared_ptr<KeyFrame> KeyFrame::getReflection(BodyModel *bodyModel) {

    std::shared_ptr<KeyFrame> reflection(new KeyFrame());
...
void KeyFrame::display() {
    ...
    cerr << "Macros:\n";
    vector< std::shared_ptr<Macro> >::iterator it( macros.begin() ), end( macros.end() );
    for( ; it != end; ++it ) {
        (*it)->display();
    }
}
...
void Skill::
appendKeyFrame( std::shared_ptr<KeyFrame> keyFrame ) {
    keyFrames.push_back( keyFrame );
    reset(); // just in case, we do it for each key frame added
}

std::shared_ptr<Skill> Skill::getReflection(BodyModel *bodyModel) {

    std::shared_ptr<Skill> reflection(new Skill());
...
void Skill::display() {
    ...
    vector< std::shared_ptr<KeyFrame> >::iterator it( keyFrames.begin() ), end( keyFrames.end() );
    for( ; it != end; ++it ) {
        (*it)->display();
    }
}
...
void Skill::editKeyframe(const int &keyframeIndex, std::shared_ptr<KeyFrame> &refKeyFrame)
{
