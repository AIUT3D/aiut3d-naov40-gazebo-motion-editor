    ...
    namedParams( namedParams_ ), rsg( rsg_ )
{
    ...
    {
        ss1.str("");
        ss1 << "/nao_dcm/"<<joint[i]<<"_position_controller/command";
        joint_command_publisher[i] = n.advertise<std_msgs::Float64>(ss1.str(), 1);
    }
    ...
    try {
        ...
        skills[SKILL_MOTION_EDITOR] = std::shared_ptr<Skill>(new Skill());
...
string NaoBehavior::Think(const std::string& message) {          
    frame_info_->frame_id++;// for removre server (ashkoofaraz)
    bodyModel->setUseOmniWalk(false);// for removre server (ashkoofaraz)
    skill = SKILL_MOTION_EDITOR;// for removre server (ashkoofaraz)
    skills[skill]->execute( bodyModel, worldModel );// for removre server (ashkoofaraz)        
    return composeAction();// for removre server (ashkoofaraz)
...
void NaoBehavior::addKeyframeToSkill(SkillType skillIndex, std::shared_ptr<KeyFrame> keyframe)
{
...
void NaoBehavior::editSkill(SkillType skillIndex, int &keyframeIndex, std::shared_ptr<KeyFrame> &newKeyframeData)
{
