...
std::shared_ptr<KeyFrame> MainWindow::createKeyframe(JointSet &js)
{    
    std::shared_ptr<KeyFrame> keyframe = std::shared_ptr<KeyFrame>(new KeyFrame());
    ...
    std::shared_ptr< Macro > setTarMacro( new SetTar( effIndices, effValues ) );
...
void MainWindow::addKeyFrameButtClicked()
{
    ...
    std::shared_ptr<KeyFrame> keyframe = createKeyframe(js);
...
void MainWindow::UpdateKeyFrameSequenceEntry(joints jointIndex, double jointVal)
{
    ...
    std::shared_ptr<KeyFrame> modifiedKeyframe = createKeyframe(*iter);
